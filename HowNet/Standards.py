...
class HowNetDict(object):
    ...
    def _expand_tree(self, tree, propertyName, layer):
        res = set()
        if layer == 0:
            return res
        target = tree
        ...
        for item in target:
            try:
                ...
                if "children" in item:
                    res |= self._expand_tree(item["children"], propertyName, layer - 1)
    ...
    def get_sememes_by_word(self, word, structured=False, lang="ch", merge=False, expanded_layer=-1):
        ...
        :param word: (str)specific word(en/ch/id) you want to search in HowNet.
        ...
        if structured:
            for item in queryResult:
                try:
                    result.append({"word": item, "tree": GenSememeTree(item["Def"])})
                except Exception as e:
            ...
            for item in queryResult:
                try:
                    if not merge:
                        result.append(
                            {"word": item[name],
                             "sememes": self._expand_tree(GenSememeTree(item["Def"]), lang, expanded_layer)})
                    else:
    ...
    def get_all_sememes(self):
        ...
        if hasattr(self, "sememe_all"):
            ...
            package_directory = os.path.dirname(os.path.abspath(__file__))
            data_dir = os.path.join(package_directory, "sememe_all.txt")
